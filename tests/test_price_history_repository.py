...
def _make_record(
    company_id: int, record_date: date, close: float = 50.0
) -> PriceHistory:
    return PriceHistory(
...
def test_create_and_get_by_id(db_session):
    ...
    record = price_history_repository.create(
        db_session, _make_record(company.id, date(2024, 1, 2))
    )
...
def test_get_by_company_ordered_desc(db_session):
    ...
    price_history_repository.create(
        db_session, _make_record(company.id, date(2024, 1, 1))
    )
    price_history_repository.create(
        db_session, _make_record(company.id, date(2024, 1, 3))
    )
...
def test_get_by_company_and_date(db_session):
    company = _make_company(db_session)
    price_history_repository.create(
        db_session, _make_record(company.id, date(2024, 1, 2), close=55.0)
    )
    result = price_history_repository.get_by_company_and_date(
        db_session, company.id, date(2024, 1, 2)
    )
    assert result is not None
...
def test_get_by_company_and_date_not_found(db_session):
    company = _make_company(db_session)
    assert (
        price_history_repository.get_by_company_and_date(
            db_session, company.id, date(2024, 1, 1)
        )
        is None
    )
...
def test_get_latest(db_session):
    company = _make_company(db_session)
    price_history_repository.create(
        db_session, _make_record(company.id, date(2024, 1, 1), close=40.0)
    )
    price_history_repository.create(
        db_session, _make_record(company.id, date(2024, 1, 5), close=60.0)
    )
