...
def test_get_by_company_ordered_desc(db_session):
    ...
    results = split_repository.get_by_company(db_session, company.id)
...
def test_get_by_company_and_date(db_session):
    ...
    split_repository.create(db_session, _make_split(company.id, date(2024, 7, 1), ratio_from=2.0, ratio_to=3.0))
    results = split_repository.get_by_company_and_date(db_session, company.id, date(2024, 7, 1))
...
def test_unique_constraint_company_split_ratio(db_session):
    company = _make_company(db_session)
    split_repository.create(db_session, _make_split(company.id, date(2024, 7, 1), ratio_from=1.0, ratio_to=2.0))
