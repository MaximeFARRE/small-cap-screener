...
def _upsert_company(session: Session, provider: BaseProvider, ticker: str, isin: str) -> Company:
    ...
    return company_repository.create(
        session,
        Company(isin=isin, ticker=ticker, name=info.name,
                sector=info.sector, market=info.market, currency=info.currency),
    )
...
def _store_prices(session: Session, provider: BaseProvider, company: Company, period: str) -> int:
    ...
    for r in records:
        ...
        price_history_repository.create(
            session,
            PriceHistory(company_id=company.id, date=r.date, open=r.open, high=r.high,
                         low=r.low, close=r.close, adjusted_close=r.adjusted_close, volume=r.volume),
        )
        added += 1
    return added
...
def _store_statements(session: Session, provider: BaseProvider, company: Company, years: int) -> int:
    ...
    for data in statements:
        ...
        financial_statement_repository.create(
            session,
            FinancialStatement(
                company_id=company.id, fiscal_year=data.fiscal_year, period_type=data.period_type,
                revenue=data.revenue, ebit=data.ebit, ebitda=data.ebitda, net_income=data.net_income,
                total_assets=data.total_assets, total_equity=data.total_equity, total_debt=data.total_debt,
                net_debt=data.net_debt, free_cash_flow=data.free_cash_flow,
...
def sync_company(
    ...
    return SyncResult(company_id=company.id, prices_added=prices_added, statements_added=statements_added)
